...
BLACKLIST = set(["401", "403", "404", "410", "genindex", "search", "contents"])
...
def get_html_path_info(root_dir: str, exclude: List[str], url: str) -> List[FileInfo]:
    ...
    for root, _, files in os.walk(root_dir):
        ...
        path_info.extend(
            [
                (root_dir, os.path.join(root, file), url)
                for file in files
...
def process_html_files(
    html_path_info: List[FileInfo], manifest: Manifest, show_progress: bool
) -> None:
...
def summarize_build(num_documents: int, start_time: float) -> None:
    ...
    summary = summary.format(num_docs=num_documents, time=str(time.time() - start_time))
    print(summary)
