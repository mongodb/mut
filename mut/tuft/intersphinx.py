...
def fetch_inventory(app, uri, inv):
    ...
    localuri = uri.find('://') == -1
    join = localuri and os.path.join or posixpath.join
    try:
        if inv.find('://') != -1:
            f = urllib2.urlopen(inv)
        else:
            f = open(os.path.join(app.srcdir, inv), 'rb')
...
def missing_reference(env, node, contnode):
    ...
    to_try = [(env.intersphqinx_inventory, target)]
    in_set = None
    if ':' in target:
